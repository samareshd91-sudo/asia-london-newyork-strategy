...
BACKTEST_KLINE_URL = "https://api-futures.kucoin.com/api/v1/kline/query"
BACKTEST_MAX_REQUEST_CANDLES = 490  # keep safely below KuCoin's 500 limit
...
def fetch_historical_1m(symbol, start_dt, end_dt, progress_callback=None):
    """
    Fetch 1-minute KuCoin Futures candles causally.

    KuCoin's current Futures REST endpoint accepts:
      symbol, granularity=60, from=<milliseconds>, to=<milliseconds>
    and returns at most 500 candles per query. We use 490-minute pages,
    advance from the last returned candle, retry transient failures, and
    validate timestamps before handing data to the backtester.
    """
    # Never request the currently-forming minute.
    ...
    while cursor <= end_ts:
        ...
        payload = _kucoin_request_json(session, params, symbol)
        rows = payload.get("data", []) or []

        if not rows:
            # No ticks can legitimately mean no candle was published.
            # Move one page forward rather than looping forever.
            cursor = chunk_end + 60
        ...
        for row in rows:
            if len(row) < 7:
                continue
            try:
                # Current Futures response uses milliseconds for start time.
        ...
        if next_cursor <= cursor:
            raise RuntimeError(
                f"KuCoin pagination stalled for {symbol}: cursor={cursor}, next={next_cursor}"
            )
        cursor = next_cursor

        if progress_callback:
            progress_callback(
                min(1.0, (cursor - start_ts) / total_seconds)
            )
        time.sleep(BACKTEST_REQUEST_PAUSE)

    if not all_rows:
        raise RuntimeError(
            f"KuCoin returned no 1-minute candles for {symbol} in the requested period."
        )
    ...
    if df.empty:
        raise RuntimeError(f"No valid candles remained after timestamp validation for {symbol}.")

    # Reject malformed OHLC rows before strategy calculations.
    ...
    if len(df) < 100:
        raise RuntimeError(
            f"Only {len(df)} valid candles downloaded for {symbol}; at least 100 are required."
        )
