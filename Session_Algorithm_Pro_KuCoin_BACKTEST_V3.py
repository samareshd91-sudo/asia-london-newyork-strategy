...
BACKTEST_KLINE_URL = "https://api.kucoin.com/api/ua/v1/market/kline"
BACKTEST_LEGACY_KLINE_URL = "https://api-futures.kucoin.com/api/v1/kline/query"
BACKTEST_MAX_REQUEST_CANDLES = 190  # UTA Futures: <=200 records/request
BACKTEST_REQUEST_PAUSE = 0.12
BACKTEST_HTTP_RETRIES = 4
BACKTEST_HTTP_BACKOFF = 0.8
BACKTEST_FEE_PCT = 0.0005       # 0.05% per side
BACKTEST_SLIPPAGE_PCT = 0.0002  # 0.02% per side
BACKTEST_INITIAL_BALANCE = 1000.0


def _kucoin_request_json(session, params, symbol):
    """GET KuCoin Futures Klines with retries and useful diagnostics."""
    last_error = None
    for attempt in range(BACKTEST_HTTP_RETRIES):
        try:
            response = session.get(
                BACKTEST_KLINE_URL,
                params=params,
                timeout=(10, 30),
                headers={"User-Agent": "Session-Algorithm-Backtester/1.0"},
            )
            try:
            ...
            if response.status_code == 200 and isinstance(payload, dict):
                ...
                last_error = RuntimeError(
                    f"KuCoin API error for {symbol}: {payload}"
                )
            else:
                detail = payload if payload is not None else response.text[:500]
                last_error = RuntimeError(
                    f"KuCoin HTTP {response.status_code} for {symbol}: {detail}"
                )
        ...
        time.sleep(BACKTEST_HTTP_BACKOFF * (2 ** attempt))

    raise RuntimeError(str(last_error) if last_error else f"Unknown KuCoin error for {symbol}")


def _kucoin_request_json_url(session, url, params, symbol):
    """GET JSON from a KuCoin public market-data endpoint with retries."""
    last_error = None
    for attempt in range(BACKTEST_HTTP_RETRIES):
        try:
            response = session.get(
                ...
                params=params,
                timeout=(10, 30),
                headers={"User-Agent": "Session-Algorithm-Backtester/2.0"},
            )
            try:
                payload = response.json()
            except ValueError:
    ...
    raise RuntimeError(
        ...
        else f"Unknown KuCoin error for {symbol}"
    )


def fetch_historical_1m(symbol, start_dt, end_dt, progress_callback=None):
    """
    Robust KuCoin Futures 1-minute historical downloader.

    Primary:
      UTA endpoint /api/ua/v1/market/kline
      tradeType=FUTURES, interval=1min
      startAt/endAt are Unix seconds
      <=200 records/request.

    Fallback:
      Legacy Futures /api/v1/kline/query
      granularity=60, from/to in milliseconds.

    Timestamp parsing is deliberately adaptive because KuCoin documentation
    and legacy/current responses have used both second- and millisecond-scale
    timestamps over time.
    """
    start_ts = int(start_dt.timestamp()) // 60 * 60
    end_ts = (int(end_dt.timestamp()) // 60 * 60) - 60
    if end_ts <= start_ts:
        return pd.DataFrame()

    page_seconds = BACKTEST_MAX_REQUEST_CANDLES * 60
    total_seconds = max(60, end_ts - start_ts + 60)
    session = requests.Session()
    all_rows = []
    cursor = start_ts
    used_legacy = False

    def normalize_rows(rows, symbol_name):
        parsed = []
        for row in rows or []:
            if len(row) < 5:
                continue
            try:
                raw_ts = int(float(row[0]))
                # 10-digit-ish => seconds; 13-digit-ish => milliseconds.
                ts_sec = raw_ts // 1000 if raw_ts >= 10**12 else raw_ts
                ...
                a = float(row[6]) if len(row) > 6 else 0.0

                if not all(np.isfinite(x) for x in (o, c, h, l, v, a)):
                    continue
                if h < max(o, c, l) or l > min(o, c, h) or v < 0:
                    continue
    ...
    # -------- Primary UTA endpoint --------
    while cursor <= end_ts:
        chunk_end = min(cursor + page_seconds - 60, end_ts)
        params = {
            "tradeType": "FUTURES",
            "symbol": symbol,
            "interval": "1min",
        ...
        try:
            payload = _kucoin_request_json_url(
                session, BACKTEST_KLINE_URL, params, symbol
            )
            rows = payload.get("data", []) or []
            parsed = normalize_rows(rows, symbol)

            if parsed:
                all_rows.extend(parsed)
                last_ts = max(x["ts"] for x in parsed)
                next_cursor = last_ts + 60
            else:
                next_cursor = chunk_end + 60

        except Exception:
            # Try the legacy Futures endpoint for this exact page.
            used_legacy = True
            legacy_params = {
                "symbol": symbol,
                "granularity": 1,
            ...
            payload = _kucoin_request_json_url(
                session, BACKTEST_LEGACY_KLINE_URL,
        ...
        if next_cursor <= cursor:
            raise RuntimeError(
                f"KuCoin pagination stalled for {symbol}: "
                f"cursor={cursor}, next={next_cursor}"
            )

        cursor = next_cursor
        if progress_callback:
            progress_callback(min(1.0, (cursor - start_ts) / total_seconds))
        time.sleep(BACKTEST_REQUEST_PAUSE)

    if not all_rows:
        raise RuntimeError(
            f"KuCoin returned 0 candles for {symbol} in the requested UTC window "
            f"{pd.to_datetime(start_ts, unit='s', utc=True)} -> "
            f"{pd.to_datetime(end_ts, unit='s', utc=True)}. "
            f"UTA was tried first and Classic Futures fallback was "
            f"{'also tried' if used_legacy else 'not needed'}."
        )
    ...
    if df.empty:
        raise RuntimeError(
            f"KuCoin returned candles for {symbol}, but none matched the "
            f"requested UTC window after timestamp normalization."
        )

    if len(df) < 100:
        raise RuntimeError(
            f"Only {len(df)} valid candles downloaded for {symbol}; "
            f"at least 100 are required."
        )
...
def run_backtest(symbols, days=30, initial_balance=1000.0,
    ...
    for n, symbol in enumerate(symbols):
        try:
            ...
            data_diagnostics.append({
                ...
                "Status": "OK",
                "Candles": len(df),
                "First Candle": str(df["datetime"].iloc[0]),
                "Last Candle": str(df["datetime"].iloc[-1]),
                "Error": "",
            })
        except Exception as exc:
    ...
    return {
        ...
        "stats": stats,
        "errors": errors,
        "data_diagnostics": pd.DataFrame(data_diagnostics),
    }
...
def render_backtest_page():
    ...
    diagnostics = result.get("data_diagnostics", pd.DataFrame())
