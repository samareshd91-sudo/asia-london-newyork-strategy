...
def fetch_historical_1m(symbol, start_dt, end_dt, progress_callback=None):
    """Download KuCoin Futures 1-minute candles with a real fallback.

    Primary: Classic Futures REST /api/v1/kline/query, granularity=60.
    Fallback: UTA /api/ua/v1/market/kline, interval=1min.
    """
    ...
    def normalize(raw_rows):
        ...
        for row in raw_rows or []:
            ...
            try:
                ...
                if h<max(o,c,l) or l>min(o,c,h) or v<0 or a<0: continue
                parsed.append({'ts':ts_sec,'open':o,'close':c,'high':h,'low':l,'volume':v,'amount':a})
    ...
    while cursor<=end_ts:
        ...
        classic_params={'symbol':symbol,'granularity':60,'from':cursor*1000,'to':chunk_end*1000}
        try:
            payload=_kucoin_request_json_url(session,BACKTEST_KLINE_URL,classic_params,symbol)
            page_rows=normalize(payload.get('data') or [])
            classic_pages+=1
        except Exception as exc:
            classic_error=exc

        # IMPORTANT: empty Classic response also triggers UTA fallback.
        if not page_rows:
            uta_params={'tradeType':'FUTURES','symbol':symbol,'interval':'1min','startAt':cursor,'endAt':chunk_end}
            try:
                payload=_kucoin_request_json_url(session,BACKTEST_UTA_KLINE_URL,uta_params,symbol)
                page_rows=normalize(payload.get('data') or [])
                uta_pages+=1
            except Exception as uta_exc:
                if classic_error is not None:
                    raise RuntimeError(f'Classic Futures failed: {classic_error}; UTA fallback failed: {uta_exc}') from uta_exc

        if page_rows:
            all_rows.extend(page_rows)
            next_cursor=max(r['ts'] for r in page_rows)+60
        else:
            next_cursor=chunk_end+60
        if next_cursor<=cursor:
            raise RuntimeError(f'Pagination stalled for {symbol}: cursor={cursor}, next={next_cursor}')
        cursor=next_cursor
    ...
    if not all_rows:
        raise RuntimeError(f'KuCoin returned 0 valid candles for {symbol}. Classic pages={classic_pages}; UTA fallback pages={uta_pages}; window={pd.to_datetime(start_ts,unit="s",utc=True)} -> {pd.to_datetime(end_ts,unit="s",utc=True)}.')
    ...
    if len(df)<100:
        raise RuntimeError(f'Only {len(df)} valid candles downloaded for {symbol}; at least 100 are required.')
